 ...
 */
var decimalToVariableLength = function(ticks) {
...
// for debugging
var decimalToHex = function(byteArray) {
    var ret = [];
    byteArray.forEach(function(decimal) {
        ret.push(decimal.toString(16));
    });
    return ret;
}
...
MidiEvent.createProgramChange = function(programNumber, channel, delay) {
    var ret = new MidiEvent({
        type: EVT_PROGRAM_CHANGE,
        channel: channel || DEFAULT_CHANNEL,
        param1: programNumber || 0,
    });
...
MidiEvent.prototype = {
    ...
    setTime: function(ticks) {
        ...
        this.time = decimalToVariableLength(ticks || 0);
    },
...
// console.log(decimalToHex(byteArray));
        return byteArray;
    }
};
...
MetaEvent.createText = function(text, typeName, delay) {
    var ret = new MetaEvent({
        type: MetaEvent.types[typeName] || META_TEXT, 
        data: asciiToCharCodes(text),
    });
...
MetaEvent.createKeySignature = function(numOfSharps, isMinor, delay) {
    var ret = new MetaEvent({
        type: META_KEY_SIG,
        data: [numOfSharps || 0, isMinor || 0],
    });
...
MetaEvent.createTimeSignature = function(numerator, denominator, clockPerClick, thirtySeconds, delay) {
    var ret = new MetaEvent({
        type: META_TIME_SIG,
        data: [
            numerator || 4,
            denominator || 2, // denominator means each beat is 1 / Math.pow(2, denominator) whole note
            clockPerClick || 18, 
            thirtySeconds || 8,
        ],
    });
...
MidiTrack.prototype = {
    ...
    toBytes: function() {
        ...
        ret = startBytes.concat(lengthBytes, eventBytes, endBytes);
// console.log(decimalToHex(ret));
...
// example
function createNonMidiTrack () {
    var noteEvents = [];
    noteEvents.push(MetaEvent.createTimeSignature());
