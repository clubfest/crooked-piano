...
Meteor.methods({
  createTranslatedSong: function(notes, notesByTrack, songInfo) {
    ...
    segmentIds.sort(function(a, b) {
      ...
      if (segmentB.text && segmentB.text.match(/melody/i)) {
        return 1;
      } else if (segmentA.text && segmentA.text.match(/melody/i)) {
        return -1;
      }

      if (segmentA.startIndex > 300 && segmentB.startIndex < 300) {
        return 1
      } else if (segmentA.startIndex < 300 && segmentB.startIndex > 300) {
        return -1;
      }
