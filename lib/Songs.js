...
Meteor.methods({
  createTranslatedSong: function(notes, segments) {
    ...
    for (var i = 0; i < notes.length; i++) {
      var idx = notes[i].segmentId;
      if (segmentIds.indexOf(idx) === -1) {
        segmentIds.push(idx);
      }
    }
  ...
  gamify: function(songId, genre) {
    ...
    // TODO: make use of segments to divide into left and right segments
    // store only the ids instead of segments
    // refactor code that depends on left and right segments
